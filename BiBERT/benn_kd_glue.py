...
class KDLearner(object):
    ...
    mm_eval_dataloader, mm_eval_labels):
        ...
        if task_name == "mnli":
            ...
            tmp_output_eval_file = os.path.join(self.args.output_dir, "eval_results.txt")
            result_to_file(result, tmp_output_eval_file)

    def train(self, train_examples, task_name, output_mode, eval_labels, 
        num_labels, train_data, eval_dataloader, eval_examples, tokenizer, save_name, size, sample_weights,
        ...
        for epoch_ in range(self.args.num_train_epochs):
            ...
            sampler = WeightedRandomSampler(sample_weights[:,0].double(), size)
                ...
                if global_step % self.args.eval_step == 0 or \
                    ...
                    if save_model:
                        ...
                        if task_name == "mnli":
                            ...
                            result = self._do_eval(self.student_model, 'mnli-mm', mm_eval_dataloader, output_mode, mm_eval_labels, num_labels)
                            result['global_step'] = global_step
                            tmp_output_eval_file = os.path.join(self.output_dir, "eval_results.txt")
                            result_to_file(result, tmp_output_eval_file)
    ...
    def _save(self,save_name=WEIGHTS_NAME):
        logging.info("******************** Save model ********************")
        model_to_save = self.student_model.module if hasattr(self.student_model, 'module') else self.student_model
        if save_name==WEIGHTS_NAME:
            output_model_file = os.path.join(self.output_dir, save_name)
        else:
            output_model_file = os.path.join(self.output_dir, save_name + '.bin')
