...
def main():
    ...
    if not args.do_eval:
        ...
        train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.batch_size, drop_last=True) # 8551*64
    ...
    eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.batch_size) # 1043*64
    if task_name == "mnli":
    ...
    if args.is_binarybert:
        student_model = BertForSequenceClassification_binary.from_pretrained(args.student_model, "pytorch_model.bin", config=student_config)
    else:
        student_model = QuantBertForSequenceClassification.from_pretrained(args.student_model, "pytorch_model.bin", config=student_config)
    ...
    if args.kd_type == 'joint_kd':
        ...
        # print("***load_state_dict:",learner.output_dir+"/pytorch_model.bin")
